...
(function () {
  ...
  function logRejectedPromises() {
    unhandledRejections.forEach(function (reason) {
      PROJECTMADHOUSE.Analytics.trackError('UnhandledPromiseRejection', reason);
    });
  ...
  function lazyLoadWCPolyfillsIfNecessary(callback) {
    ...
    if (!(PROJECTMADHOUSE.Util.getChromeVersion() &&
      PROJECTMADHOUSE.Util.getChromeVersion() >= 46 || PROJECTMADHOUSE.Util.getFirefoxVersion() && PROJECTMADHOUSE.Util.getFirefoxVersion() >= 40)) {
      var parent = document.querySelector('body');
  ...
  window.addEventListener('offline', function () {
    if (PROJECTMADHOUSE.Elements && PROJECTMADHOUSE.Elements.Template.$.toast) {
      PROJECTMADHOUSE.Elements.Template.$.toast.show(
  ...
  window.addEventListener('beforeinstallprompt', function (event) {
    PROJECTMADHOUSE.Analytics.trackEvent('installprompt', 'fired');

    event.userChoice.then(function (choiceResult) {
      // choiceResult.outcome will be 'accepted' or 'dismissed'.
      // choiceResult.platform will be 'web' or 'android' if the prompt was
      // accepted, or '' if the prompt was dismissed.
      PROJECTMADHOUSE.Analytics.trackEvent('installprompt', choiceResult.outcome,
